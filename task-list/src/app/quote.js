export const localQuotes = [
  {
    text: "Today is the tomorrow we worried about yesterday.",
    author: "Dale Carnegie",
  },
  {
    text: "Practice does not make perfect. Practice makes permanent.",
    author: "Sarah Key",
  },
  {
    text: "The secret to having it all, is knowing that you already do.",
    author: "Koh x",
  },
  {
    text: "The only thing we have to fear is fear itself.",
    author: "Franklin D. Roosevel",
  },
  {
    text: "Be yourself; everyone else is already taken.",
    author: "Oscar Wilde",
  },
  {
    text: "May you live all the days of your life.",
    author: "Jonathan Swift",
  },
  {
    text: "Life is trying things to see if they work",
    author: "Ray Bradbury",
  },
  {
    text: "Everything you can imagine is real.",
    author: "Pablo Picasso",
  },
  {
    text: "Don't be afraid to give up the good to go for the great.",
    author: "John D. Rockefeller",
  },
  {
    text: "By failing to prepare, you are preparing to fail.",
    author: "Benjamin Franklin",
  },
  {
    text: "No matter what happens, or how bad it seems today, life does go on, and it will be better tomorrow.",
    author: "Maya Angelou",
  },
  {
    text: "Instead of being ashamed of what you’ve been through, be proud of what you have overcome.",
    author: "Dr. Phil",
  },
  {
    text: "There is beauty in the journey, regardless of the outcome. ",
    author: "Marie White",
  },
  {
    text: "I am not what happened to me. I am what I choose to become.",
    author: "Carl Jung",
  },
  {
    text: "You will never be able to escape from your heart. So it is better to listen to what it has to say.",
    author: "Paulo Coelho",
  },
  {
    text: "At the end of the day, we can endure much more than we think we can.",
    author: "Frida Kahlo",
  },
  {
    text: "Nothing is absolute. Everything changes, everything moves, everything revolves, everything flies and goes away.",
    author: "Frida Kahlo",
  },
  {
    text: "When you want something, all the universe conspires in helping you to achieve it.",
    author: "Paulo Coelho",
  },
  {
    text: "Be brave. Take risks. Nothing can substitute experience.",
    author: "Paulo Coelho",
  },
  {
    text: "You have to take risks. We will only understand the miracle of life fully when we allow the unexpected to happen.",
    author: "Paulo Coelho",
  },
  {
    text: "When a person really desires something, all the universe conspires to help that person to realize his dream.",
    author: "Paulo Coelho",
  },
  {
    text: "When a person really desires something, all the universe conspires to help that person to realize his dream.",
    author: "Paulo Coelho",
  },
  {
    text: "The more violent the storm, the quicker it passes.",
    author: "Paulo Coelho",
  },
  {
    text: "Tell your heart that the fear of suffering is worse than the suffering itself. ",
    author: "Paulo Coelho",
  },
  {
    text: "Everything that irritates us about others can lead us to an understanding of ourselves.",
    author: "Carl Jung",
  },
  {
    text: "Where love rules, there is no will to power; and where power predominates, there love is lacking.",
    author: "Carl Jung",
  },
  {
    text: "One good thing about music, when it hits you, you feel no pain.",
    author: "Bob Marley",
  },
  {
    text: "You may not control all the events that happen to you, but you can decide not to be reduced by them.",
    author: "Maya Angelou",
  },
];
